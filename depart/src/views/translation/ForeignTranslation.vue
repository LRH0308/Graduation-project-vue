<template>
  <div class="foreign-translation-container">
    <el-card>
      <template #header>
        <div class="card-header">
          <span>外文翻译管理</span>
          <el-button type="primary" @click="getForeignTranslationList" :loading="loading">
      ...
      <!-- 外文翻译列表 -->
      <el-table :data="foreignTranslationList" v-loading="loading" border stripe>
      ...
      <el-pagination
        ...
        @current-change="getForeignTranslationList"
        @size-change="getForeignTranslationList"
    ...
    <el-dialog v-model="detailDialogVisible" title="外文翻译详情" width="700px">
      <el-descriptions :column="1" border>
        <el-descriptions-item label="选题">{{ currentTranslation.projectName }}</el-descriptions-item>
        <el-descriptions-item label="学生姓名">{{ currentTranslation.studentName }}</el-descriptions-item>
        <el-descriptions-item label="学生学号">{{ currentTranslation.studentAccount }}</el-descriptions-item>
        ...
        <el-descriptions-item label="导师姓名">{{ currentTranslation.teacherName }}</el-descriptions-item>
        <el-descriptions-item label="导师工号">{{ currentTranslation.teacherAccount }}</el-descriptions-item>
        <el-descriptions-item label="审核时间">{{ currentTranslation.auditTime || '无' }}</el-descriptions-item>
        <el-descriptions-item label="审核意见" :span="2">{{ currentTranslation.auditRemark || '无' }}</el-descriptions-item>
      </el-descriptions>
          ...
          <el-descriptions-item label="外文原文">
            <div class="file-item">
              ...
              <el-button
                ...
                link
                size="small"
                @click="handleDownload(currentTranslation.originalFileId, fileNames.originalFileName || '外文原文')"
              >
          ...
          <el-descriptions-item label="外文翻译">
            <div class="file-item">
              ...
              <el-button
                ...
                link
                size="small"
                @click="handleDownload(currentTranslation.translateFileId, fileNames.translateFileName || '外文翻译')"
              >
                <el-icon><Download /></el-icon>
                下载
              </el-button>
...
const getForeignTranslationList = async () => {
  ...
  try {
    ...
    if (response?.status === 'success') {
      foreignTranslationList.value = response.data?.records || response.data?.list || []
      total.value = response.data?.total || 0
    } else {
      ElMessage.warning('获取外文翻译列表失败')
    }
  } catch (error) {
